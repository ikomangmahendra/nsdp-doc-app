...
export class SectionComponent implements OnInit {
  ...
  trackId = (item: ISection): string => this.sectionService.getSectionIdentifier(item);
      ...
      .pipe(
        ...
        tap(() => {
          if (this.sections().length === 0) {
            ...
            this.sections.set(this.refineData(this.sections()));
          }
        }),
      )
      .subscribe();
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.sections.set(this.refineData(dataFromBody));
  }

  protected refineData(data: ISection[]): ISection[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ISection[] | null): ISection[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.sectionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
